    ...
    QMainWindow(parent)
{
    ...
    QGraphicsScene *scene = new QGraphicsScene(this);
        ...
        chariot = new QGraphicsRectItem(newPos, 20, 50, 20);
        roue1 = new QGraphicsEllipseItem(newPos - 5, 35, 15, 15);
        roue2 = new QGraphicsEllipseItem(newPos + 40, 35, 15, 15);
        pendule = new QGraphicsLineItem(newPos + 25, 20, newPos + 130*sin(angle) + 15, 20 + 130*cos(angle));
        rail = new QGraphicsRectItem(-100, 50, 300, 10);

        scene->addItem(chariot);
        scene->addItem(roue1);
        scene->addItem(roue2);
...
void MainWindow::receiveFromSerial(QString msg){
    ...
    if(msgBuffer_.endsWith('\n')){
        ...
        if(~jsonResponse.isEmpty()){
            ...
            ui->textBrowser2->setText(buff.mid(2,buff.length()-4));

            angle = jsonObj["degresPendule"].toDouble();
            newPos = jsonObj["position"].toDouble();

            deltaPos = newPos - prevPos;

            trans_robot.translate(deltaPos, 0);
            //trans_pendule.translate(position + 130*sin(angle), 20 - 130*cos(angle));
            chariot->setTransform(trans_robot);
            roue1->setTransform(trans_robot);
            roue2->setTransform(trans_robot);
            pendule->setTransformOriginPoint(18, 20);
            //pendule->p1->setTransform(trans_pendule);
            pendule->setRotation(angle);
            pendule->setTransform(trans_robot);
...
void MainWindow::connectButtons(){
    ...
    connect(ui->Start, SIGNAL(clicked()), this, SLOT(sendBoolStart()));
    connect(ui->Stop, SIGNAL(clicked()), this, SLOT(sendBoolStop()));
    connect(ui->AimantOn, SIGNAL(clicked()), this, SLOT(sendBoolAimantOn()));
    connect(ui->AimantOff, SIGNAL(clicked()), this, SLOT(sendBoolAimantOff()));
    connect(ui->StopAll, SIGNAL(clicked()), this, SLOT(sendBoolStopAll()));
}
...
void MainWindow::sendBoolAimantOn()
{
    QJsonObject jsonObject
    {
        {"AimantOn", 1}
...
void MainWindow::sendBoolAimantOff()
{
    QJsonObject jsonObject
    {
        {"AimantOff", 1}
...
void MainWindow::sendBoolStopAll()
{
    QJsonObject jsonObject
    {
        {"StopAll", 1}
